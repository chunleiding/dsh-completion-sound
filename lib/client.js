	...
	factory: (require) => {
		...
		*/
		function CompletionSoundSection(props) {
			const { t, useStore, setEnabled, setNotify, setVolume, setLongTaskMinutes, setSpecial, setSpecialPath, setAskAlert, previewChime, previewAsk, previewSpecial, testNotify } = props;
		...
		function apply(ctx) {
			...
			*/
			...
			ctx.inject(["uiSession"], (sessionCtx) => {
				...
				const prime = () => {
					const next = /* @__PURE__ */ new Map();
					for (const [id, interaction] of sessionCtx.uiSession.pendingInteractions.getSnapshot()) next.set(id, interaction.key);
					return next;
				};
				seen = prime();
				sessionCtx.effect(() => sessionCtx.uiSession.pendingInteractions.subscribe(() => {
					...
					if (!coordinator.isLeader()) return;
					playAttentionChime(settings.volume);
					const title = arrivals.map((a) => a.title).join(", ");
					notifyCompletion(t(arrivals.some((a) => a.approving) ? "completion-sound.askedApproval" : "completion-sound.asked"), title);
