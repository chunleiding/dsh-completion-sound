			...
			ctx.inject(["uiSession"], (sessionCtx) => {
				...
				const armRepeat = (id) => {
					...
					entry.timer = setTimeout(() => {
						entry.timer = null;
						const interaction = sessionCtx.uiSession.pendingInteractions.getSnapshot().get(id);
				...
				for (const [id, interaction] of sessionCtx.uiSession.pendingInteractions.getSnapshot()) awaiting.set(id, {
					key: interaction.key,
					approving: interaction.kind === "approval",
					timer: null
				});
