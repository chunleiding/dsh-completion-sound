...
const CompletionSoundSettingsSchema = z.object({
	...
	[COMPLETION_SOUND_ASK_REPEAT_FIELD]: z.number().min(0).max(MAX_ASK_REPEAT_MINUTES).default(3),
...
function apply(ctx) {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		settings = settingsCtx.settings.register(COMPLETION_SOUND_SETTINGS_NAMESPACE, CompletionSoundSettingsSchema);
	});
	ctx.inject(["webServer"], (webCtx) => {
		...
		webCtx.effect(() => webCtx.webServer.register({
			...
			handler: cueRoute((values) => values[COMPLETION_SOUND_SPECIAL_PATH_FIELD], true)
		}), "ui-completion-sound: special-cue resolver route");
		webCtx.effect(() => webCtx.webServer.register({
			kind: "exact",
			path: ASK_SOUND_URL,
			handler: cueRoute((values) => values[COMPLETION_SOUND_ASK_PATH_FIELD], false)
...
//#endregion
export { ASK_SOUND_URL, COMPLETION_SOUND_ASK_ALERT_FIELD, COMPLETION_SOUND_ENABLED_FIELD, COMPLETION_SOUND_LONG_TASK_MINUTES_FIELD, COMPLETION_SOUND_NOTIFY_FIELD, COMPLETION_SOUND_NOTIFY_URL, COMPLETION_SOUND_SETTINGS_NAMESPACE, COMPLETION_SOUND_SPECIAL_FIELD, COMPLETION_SOUND_SPECIAL_PATH_FIELD, COMPLETION_SOUND_VOLUME_FIELD, DEFAULT_ASK_REPEAT_MINUTES, DEFAULT_LONG_TASK_MINUTES, DEFAULT_VOLUME, GUAN_YU_SOUND_URL, MAX_ASK_REPEAT_MINUTES, MAX_LONG_TASK_MINUTES, MIN_ASK_REPEAT_MINUTES, MIN_LONG_TASK_MINUTES, SPECIAL_SOUND_URL, apply };
